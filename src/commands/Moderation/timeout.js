...
module.exports = {
    ...
    async execute(interaction, message, client) {
        ...
        if (!timeMember) return await interaction.reply({ content: '<:Denied:1125943015878443089> The user mentioned is no longer within the server.', ephemeral: true})
        if (!timeMember.kickable) return interaction.reply({ content: '<:Denied:1125943015878443089> I cannot timeout this user! This is either because their higher than me or you.', ephemeral: true})
        if (!duration) return interaction.reply({content: 'You must set a valid duration for the timeout', ephemeral: true})
        if (interaction.member.id === timeMember.id) return interaction.reply({content: "<:Denied:1125943015878443089> You cannot timeout yourself!", ephemeral: true})
        if (timeMember.permissions.has(PermissionsBitField.Flags.Administrator)) return interaction.reply({content: "<:Denied:1125943015878443089> You cannot timeout staff members or people with the Administrator permission.", ephemeral: true})
        ...
        await timeMember.timeout(duration * 1000, reason)
            ...
            .setColor("Orange")
            .setDescription(`<:MutedIcon:1128102550424518667> | ${timeUser.tag} has been **timed out** for ${duration / 60} minute(s) | ${reason}`)
 
            const dmEmbed = new EmbedBuilder()
            .setDescription(`<:MutedIcon:1128102550424518667> | You have been **timed out** in ${interaction.guild.name}. [You can check the status of your timeout within the server.] | ${reason}`)
            .setColor(`Orange`)
